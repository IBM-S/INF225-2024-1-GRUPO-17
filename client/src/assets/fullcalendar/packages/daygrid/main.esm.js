...
function renderCellHtml(date, dateProfile, context, otherAttrs) {
    ...
    var formattedDate = dateEnv.formatIso(date, { omitTime: true });
    return '<form id="fecha" action="info_dia.html" method="post">' +
        '<input type="hidden" name="fecha" value="' + formattedDate + '">' +
        '<td class="' + classes.join(' ') + '">' +
        '<button type="submit" style="background: transparent; border: none; cursor: pointer; width: 100%; height: 100%;" data-date="' + formattedDate + '"></button>' +
        '</td>' +
        '</form>';
